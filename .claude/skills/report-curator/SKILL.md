---
name: report-curator
description: Maintain this project's living scientific documentation — FOUR LaTeX docs from a shared docs/figures/make_figures.py: docs/experiments.tex + docs/experiments.es.tex (English + Spanish, the ENTIRE experiment series) and docs/next-track.tex + docs/next-track.es.tex (English + Spanish, the NEXT-TRACK family only) — consolidating the experiments/*.md campaign reports. Spawns the report-curator agent. Use when the user wants to update or sync the experiment PDFs, fold new experiment reports in, refresh the Spanish translation, add a figure (heatmaps for parameter scans), extend the theory/background, refresh a leaderboard, polish prose, or rebuild.
user-invocable: true
argument-hint: "[sync|polish|rebuild] [extra instructions]"
allowed-tools:
  - Task
---
<!-- GENERATED from agents-src/skills/report-curator/SKILL.md by agents-src/render.py — edit the template (and domain.toml), not this file; then run `zig build render-agents`. -->
...
|---|---|---|
| **sync** (default) | Per document, detect in-scope `experiments/*.md` reports not yet reflected, fold each in additively (section + transcribed tables + figures + leaderboard/abstract refresh), mirror into the Spanish twin, rebuild all four PDFs | proceeds freely |
...
abstract's thesis, or archiving/moving any `experiments/*.md`. Creating a
