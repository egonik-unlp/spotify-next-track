...
domain (`domain.toml`), its empirical record (`experiments/`, `models.toml`,
...
- Check `GET http://localhost:8096/api/runs` before starting and report any live
...
- You never write `domain.toml`, `models.toml`, `registry.toml`,
  `experiments/PROJECT-FACTS.md`, anything under `experiments/` or `data/`, or the
