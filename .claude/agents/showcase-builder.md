...
```sh
curl -s localhost:8096/api/models                      # is <name> in here?
curl -s localhost:8096/api/models/<name>/contract      # input fields, dims, transform, dataset
curl -s localhost:8096/api/domain                       # nouns, target.format, the input field schema
```
...
```sh
curl -fsS -OJ localhost:8096/api/models/<name>/export    # -> <name>-export.tar.gz
...
The embedding (`200`-dim) is the one input the bundle can't
...
curl -s localhost:8096/api/datasets/<dataset_id>/split | python3 -c "import json,sys; print(json.load(sys.stdin))" | head
```
...
| field | notes |
|---|---|
| `engagement` | engagement (former target); plain number. |
| `embedding_source` | corpus filter field — match the corpus convention. |
| `play_count` | play count; plain number. |
| `completion_ratio` | completion; plain number. |
| `skip_rate` | skip rate; plain number. |
| `shuffle_rate` | shuffle rate; plain number. |
| `distinct_sessions` | distinct sessions; plain number. |
| `artist_popularity` | artist popularity; plain number. |
| `track_popularity` | track popularity; plain number. |
| `artist_followers` | artist followers; plain number. |
| `release_year` | release year; plain number. |
| `artist_count` | credited artists; plain number. |
| `af_danceability` | danceability; plain number. |
| `af_energy` | energy; plain number. |
| `af_valence` | valence; plain number. |
| `af_acousticness` | acousticness; plain number. |
| `af_instrumentalness` | instrumentalness; plain number. |
| `af_speechiness` | speechiness; plain number. |
| `af_liveness` | liveness; plain number. |
| `af_loudness` | loudness (dB); plain number. |
| `af_tempo` | tempo (BPM); plain number. |
| `af_key` | key (pitch class); plain number. |
| `af_mode` | mode (maj/min); plain number. |
| `is_saved` | saved to library; plain number. |
| `in_playlist_count` | playlist memberships; plain number. |
| `on_repeat_count` | on-repeat snapshots; plain number. |
| `first_played` | first played; omit; the server stamps it. |
| `genre_primary` | primary genre; categorical, as on the page. |
| `artist` | categorical, as on the page. |
| `album_type` | release type; categorical, as on the page. |
| `top_platform` | top platform; categorical, as on the page. |
...
- Cross-check correctness against the live server on the same item:
  `POST localhost:8096/api/models/<name>/predict` (server featurizer + native
