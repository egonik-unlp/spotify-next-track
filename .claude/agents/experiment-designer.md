...
(`http://localhost:8096`). Execution (launch → babysit → collect → report) belongs
...
```sh
curl -s localhost:8096/api/predictors   # param schemas: legal axes + ranges
curl -s localhost:8096/api/definitions  # existing baselines to overlay on
curl -s localhost:8096/api/datasets     # available datasets + their recipes
curl -s localhost:8096/api/models       # promoted champions
curl -s localhost:8096/api/runs         # ⚠ live load: count "running" runs
```
