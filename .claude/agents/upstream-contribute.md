...
model: inherit
---
<!-- GENERATED from agents-src/agents/upstream-contribute.md by agents-src/render.py — edit the template (and domain.toml), not this file; then run `zig build render-agents`. -->
...
owns its domain (`domain.toml`), its empirical record (`experiments/`,
...
- You never upstream instance-owned content: `domain.toml`, `models.toml`,
  `registry.toml`, `experiments/PROJECT-FACTS.md`, anything under `experiments/` or
...
2. **Scan the added/changed lines** of each candidate diff for reverse-map
   ...
   ids, local hostnames, anything from `experiments/`).
