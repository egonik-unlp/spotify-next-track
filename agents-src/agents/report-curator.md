...
reports in `{{report_dir}}/*.md` into rationale, interpretation, theory, and
...
- `docs/experiments.tex` — **English, the ENTIRE experiment series** (every
  campaign in `{{report_dir}}/*.md`: the inherited rotation lineage AND the
...
You NEVER edit `{{report_dir}}/*.md` — those are the source of truth, owned by
