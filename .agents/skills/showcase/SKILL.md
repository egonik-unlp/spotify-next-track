...
allowed-tools:
  ...
  - Bash(curl *)
---
<!-- GENERATED from agents-src/skills/showcase/SKILL.md by agents-src/render.py — edit the template (and domain.toml), not this file; then run `zig build render-agents`. -->
...
must be running, default `http://localhost:8096`):
...
- **Mode** — how visitors interact:
  - **curated** *(default)* — a fixed gallery of real example tracks
