...
allowed-tools:
  ...
  - Bash(python3 *)
---
<!-- GENERATED from agents-src/skills/model-export/SKILL.md by agents-src/render.py — edit the template (and domain.toml), not this file; then run `zig build render-agents`. -->
...
```bash
curl -fsS -OJ http://localhost:8096/api/models/<name>/export    # -> <name>-export.tar.gz
...
<name>/
  ...
  pca_components.f32     PCA basis, row-major float32[dims, 200]
...
The embedding (`200`-dim) is supplied by the caller — it comes
...
`POST http://localhost:8096/api/models/<name>/predict` (server featurizer + native
