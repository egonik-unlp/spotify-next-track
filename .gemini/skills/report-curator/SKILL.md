---
name: report-curator
description: Maintain the single living scientific report (docs/experiments.tex + docs/figures/make_figures.py → docs/experiments.pdf) that consolidates the experiments/*.md campaign reports. Spawns the report-curator agent. Use when the user wants to update or sync the experiment PDF, fold new experiment reports into the document, add a figure (heatmaps for parameter scans), add or extend the theory/background section, refresh the leaderboard, polish the prose, or rebuild the report.
user-invocable: true
...
|---|---|---|
| **sync** (default) | Detect `experiments/*.md` reports not yet reflected in the document, fold each in additively (section + transcribed tables + figures + leaderboard/abstract refresh), rebuild the PDF | proceeds freely |
...
thesis, or archiving/moving any `experiments/*.md`. Additive work never
stops.
